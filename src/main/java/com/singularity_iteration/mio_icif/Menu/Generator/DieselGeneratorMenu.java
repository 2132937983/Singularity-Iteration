...
public class DieselGeneratorMenu extends mio_icif_generator_menu {
    ...
    public DieselGeneratorMenu(int containerId, Inventory playerInventory,
                                @Nullable mio_icif_diesel_generator blockEntity,
                                @Nullable IItemHandler itemHandler,
                                @Nullable ContainerData data) {
        super(mio_icif_menus.DIESEL_GENERATOR_MENU_TYPE.get(), containerId, SLOT_COUNT,
    ...
    protected void addMachineSlots() {
        this.addSlot(new SlotItemHandler(itemHandler, FUEL_BUCKET_SLOT, 27, 21) {
            ...
            public boolean mayPlace(ItemStack stack) {
                boolean isFuel = stack.is(mio_icif_fluids.DIESELOIL_BUCKET.get()) || mio_icif_cells.isCellContainingFluid(stack, mio_icif_fluids.DIESELOIL.get());
                if (!isFuel) return false;
                if (blockEntity instanceof mio_icif_diesel_generator gen && gen.getFuelAmount() >= gen.getFuelCapacity()) return false;
                return true;
            }
        });
    ...
    protected boolean isValidFuel(ItemStack stack) {
        return stack.is(mio_icif_fluids.DIESELOIL_BUCKET.get()) || mio_icif_cells.isCellContainingFluid(stack, mio_icif_fluids.DIESELOIL.get());
    }
    ...
    public void broadcastChanges() {
        ...
        if (blockEntity instanceof mio_icif_diesel_generator gen) {
