...
public class mio_icif_gui_advanced_solar_panel extends mio_icif_screen<AdvancedSolarPanelMenu> {
    ...
    public mio_icif_gui_advanced_solar_panel(AdvancedSolarPanelMenu menu, Inventory playerInventory, Component title) {
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        ...
        AdvancedSolarPanelMenu menu = this.getMenu();
        if (menu != null) {
            // 绘制 EU 能量条
        ...
        if (mouseX >= x + SUN_ICON_X && mouseX <= x + SUN_ICON_X + SUN_ICON_WIDTH &&
            ...
            if (menu.isGenerating()) {
                if (menu.isDaytimeGeneration()) {
                    tooltip = Component.literal("Generating: 8 EU/t (Day)");
                } else {
                    tooltip = Component.literal("Generating: 1 EU/t (Night)");
                }
            } else {
