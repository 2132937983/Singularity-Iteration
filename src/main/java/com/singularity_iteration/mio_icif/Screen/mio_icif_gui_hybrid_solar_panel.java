...
public class mio_icif_gui_hybrid_solar_panel extends mio_icif_screen<HybridSolarPanelMenu> {
    ...
    public mio_icif_gui_hybrid_solar_panel(HybridSolarPanelMenu menu, Inventory playerInventory, Component title) {
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        ...
        HybridSolarPanelMenu menu = this.getMenu();
        if (menu != null) {
            // 绘制 EU 能量条
        ...
        if (mouseX >= x + SUN_ICON_X && mouseX <= x + SUN_ICON_X + SUN_ICON_WIDTH &&
            ...
            if (menu.isGenerating()) {
                if (menu.isDaytimeGeneration()) {
                    tooltip = Component.literal("Generating: 64 EU/t (Day)");
                } else {
                    tooltip = Component.literal("Generating: 8 EU/t (Night)");
                }
            } else {
