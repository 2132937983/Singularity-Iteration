...
public class mio_icif_dynamite_item extends Item {

    public mio_icif_dynamite_item(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            mio_icif_dynamite_entity dynamite = new mio_icif_dynamite_entity(level, player.getX(), player.getY(0.5), player.getZ(), player);
    ...
    public static void registerDispenserBehavior() {
        DispenserBlock.registerBehavior(mio_icif_normal.DYNAMITE.get(), new DefaultDispenseItemBehavior() {
            @Override
            protected ItemStack execute(BlockSource source, ItemStack stack) {
                ...
                mio_icif_dynamite_entity dynamite = new mio_icif_dynamite_entity(level, position.x(), position.y(), position.z(), null);
                double speed = 0.8;
