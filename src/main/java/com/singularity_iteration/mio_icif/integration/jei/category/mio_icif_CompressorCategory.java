 ...
 * 压缩�?JEI 配方类别
 */
public class mio_icif_CompressorCategory implements IRecipeCategory<mio_icif_CompressorRecipe> {

    public static final ResourceLocation UID = ResourceLocation.fromNamespaceAndPath(Singularity_Iteration.MOD_ID, "compressor");
    public static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(Singularity_Iteration.MOD_ID,
            "textures/gui/gui_compressor_elc.png");
    public static final ResourceLocation ATLAS_TEXTURE = mio_icif_gui_global_variables.ATLAS_TEXTURE;
    public static final RecipeType<mio_icif_CompressorRecipe> COMPRESSOR_TYPE =
            new RecipeType<>(UID, mio_icif_CompressorRecipe.class);

    private final IDrawable background;
    private final IDrawable icon;

    public mio_icif_CompressorCategory(IGuiHelper helper) {
        // 背景只绘制机器内容区域，去掉边框
        this.background = helper.createDrawable(TEXTURE, 7, 7, 162, 72);
        this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK,
                new ItemStack(mio_icif_blocks.COMPRESSOR_ELC.get()));
    }

    @Override
    public RecipeType<mio_icif_CompressorRecipe> getRecipeType() {
        return COMPRESSOR_TYPE;
    }

    @Override
    public Component getTitle() {
        return Component.translatable("jei.mio_icif.compressor");
    }
    ...
    @Override
    public void setRecipe(IRecipeLayoutBuilder builder, mio_icif_CompressorRecipe recipe, IFocusGroup focuses) {
        // 槽位坐标已根据新的背景区域(7,7)调整
        // 修复：使用 addItemStacks 并设置正确的输入数量
    ...
    @Override
    public void draw(mio_icif_CompressorRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        this.background.draw(guiGraphics, 0, 0);
        
        // 绘制压缩机专属进度条 - 使用与 Screen 基类相同的方法
        int progressX = 72;
        int progressY = 27;
        int progressPixels = mio_icif_gui_global_variables.COMPRESSOR_WORK_PROGRESS_U; // 满进度
